...
const readClientDocumentsTool: Tool = {
  ...
  description:
    "Scan a client's OneDrive folder for documents. Returns the file list with names, sizes, dates, and detected document types (W-2, 1099, K-1, etc.). Use this as the first step in any document ingestion workflow.",
  input_schema: {
    type: "object" as const,
    properties: {
      client_id: { type: "string", description: "Client UUID or client name (will be resolved automatically)" },
      folder: { type: "string", description: "Specific subfolder path to scan (e.g., 'Tax/2025', 'Year End Tax'). Omit to scan the root client folder." },
      file_types: {
        type: "array",
        items: { type: "string" },
        description: "Filter by file extensions (e.g., ['pdf', 'xlsx']). Omit to show all files.",
      },
    },
    required: ["client_id"],
  },
...
export async function executeTool(
  ...
  switch (toolName) {
    ...
    case "send_partner_alert": {
      ...
      // Send email alert to both partners
      try {
        ...
        const { data: agentRecord } = await supabaseAdmin
          ...
          .single();
        const agentName = (agentRecord?.name as string) ?? "AI Agent";

        const alertTitle = input.title as string ?? (input.alert_type as string);
        const alertMessage = input.message as string;
        const alertPriority = (input.alert_type as string) ?? "update";

        await sendEmail(context.partnerId, {
          to: [
            { name: "Emily Chen", address: "emily.chen@evergrowfin.com" },
            { name: "Cara Jiang", address: "jiangy@evergrowfin.com" },
          ],
          subject: `\u26a0\ufe0f Agent Alert: ${alertTitle}`,
          body: `<div style="font-family: Arial, sans-serif; max-width: 600px;">
  <h2 style="color: #b45309;">\u26a0\ufe0f Agent Alert</h2>
  <table style="border-collapse: collapse; width: 100%; margin-bottom: 16px;">
    <tr>
      <td style="padding: 8px; font-weight: bold; border-bottom: 1px solid #e5e7eb;">Agent</td>
      <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${agentName}</td>
    </tr>
    <tr>
      <td style="padding: 8px; font-weight: bold; border-bottom: 1px solid #e5e7eb;">Priority</td>
      <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${alertPriority.toUpperCase()}</td>
    </tr>
    <tr>
      <td style="padding: 8px; font-weight: bold; border-bottom: 1px solid #e5e7eb;">Message</td>
      <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${alertMessage}</td>
    </tr>
  </table>
  <p style="color: #6b7280; font-size: 12px;">This alert was generated by the Evergrow AI Agent Platform.</p>
</div>`,
          bodyType: "HTML",
        });
      } catch (emailErr) {
        // Don't break the tool if Microsoft isn't connected
        console.error("Failed to send partner alert email:", emailErr);
      }
    ...
    case "read_client_documents": {
      ...
      const rawClientId = input.client_id as string ?? input.client_name as string;
      if (!rawClientId) return "Error: client_id (or client_name) is required.";

      // Resolve client name/ID to a UUID
      const resolvedDocClientId = await resolveClientId(rawClientId, context.clientId);
      if (!resolvedDocClientId) return `Could not find a client matching "${rawClientId}". Please provide the exact client name or ID.`;

      // Get client name and type from Supabase
      const { data: clientRecord, error: clientError } = await supabaseAdmin
        .from("clients")
        .select("name, type")
        .eq("id", resolvedDocClientId)
        .single();
      if (clientError || !clientRecord) return `Error looking up client: ${clientError?.message ?? "Client not found."}`;

      const clientName = clientRecord.name as string;
      const clientType = (clientRecord.type as "business" | "individual") ?? "individual";
      const folder = input.folder as string | undefined;
      const fileTypes = input.file_types as string[] | undefined;

      const scanResult = await scanClientFolder(context.partnerId, clientName, clientType);

      // If a specific folder was requested, scan that subfolder directly
      let folderResult: string;
      if (folder) {
        const fullPath = `${buildClientPath(clientName, clientType)}/${folder}`;
        const listResult = await listFolder(context.partnerId, fullPath);
        if (!listResult.ok) {
          folderResult = `Could not access folder "${folder}": ${listResult.error}`;
        } else if (!listResult.items?.length) {
          folderResult = `Folder "${folder}" is empty.`;
        } else {
          // Format the file list
          const lines: string[] = [];
          for (const item of listResult.items) {
            if (item.folder) {
              lines.push(`  [Folder] ${item.name}/ (${item.folder.childCount} items)`);
            } else {
              // Apply file_types filter if provided
              if (fileTypes && fileTypes.length > 0) {
                const ext = item.name.split(".").pop()?.toLowerCase() ?? "";
                if (!fileTypes.some((ft: string) => ft.toLowerCase() === ext)) continue;
              }
              const modified = new Date(item.lastModifiedDateTime).toLocaleDateString();
              const sizeKB = (item.size / 1024).toFixed(1);
              const sizeMB = (item.size / 1024 / 1024).toFixed(1);
              const sizeStr = item.size > 1024 * 1024 ? `${sizeMB} MB` : `${sizeKB} KB`;
              lines.push(`  ${item.name}  (${sizeStr}, modified ${modified})`);
            }
          }
          folderResult = lines.length > 0
            ? `Documents in ${clientName}/${folder} (${lines.length} items):\n\n${lines.join("\n")}`
            : `No files matching the specified types found in "${folder}".`;
        }
      } else {
        folderResult = scanResult;
      }

      // Add document type analysis
      const docTypes: string[] = [];
      const lower = folderResult.toLowerCase();
      ...
      return folderResult + analysis + "\n\nUse the individual file reading tools to extract data from each document.";
    }

    default:
      return `Unknown tool: ${toolName}. This tool is not implemented.`;
  }
}
